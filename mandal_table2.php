...
$i=0;
	...
	while($n--)
	{
		...
		$sql="select * from z_scheme_".$schemes[$i]." where m_id='$m_id'";
		$query=mysql_query($sql);
		$data=mysql_fetch_array($query);
		if($data)
		{
			
			$q=mysql_query("select * from scheme_details where id='$schemes[$i]'");
			$q=mysql_fetch_array($q);
			echo '<tr>';
			echo '<td>'.++$i.'</td>';
			echo '<td><a target="_blank" href="scheme_details_mro.php?id='.$q['id'].'">'.$q['name'].'</a></td>';
